module Xingfus
  USER_AGENT = "Xingfus - http://github.com/dbalatero/xingfus/tree/master"
  ...
  module ClassMethods
    ...
    # set of Xingfus options to be used to store/retrieve
    ...
    [:get, :post, :put, :delete].each do |method|
      ...
      SRC
      module_eval(code, "./lib/xingfus/remote.rb", line)
    end
    
    def remote_proxy_object(url, method, options)
      easy = Xingfus.get_easy_object
      
      easy.url                   = url
      easy.method                = method
      easy.headers               = options[:headers] if options.has_key?(:headers)
      easy.headers["User-Agent"] = (options[:user_agent] || Xingfus::USER_AGENT)
      ...
      proxy = Xingfus::RemoteProxyObject.new(clear_memoized_proxy_objects, easy, options)
