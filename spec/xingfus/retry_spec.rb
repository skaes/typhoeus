...
describe Xingfus::Request do
  describe '#on_retry' do
    it 'should set the @on_retry to the supplied block' do
      some_value = false
      request = Xingfus::Request.new('url')
  ...
  describe '#call_retry_handler' do
    it 'should call the on_retry handler with the request' do
      ...
      response = Xingfus::Response.new(:code => '999')
      request = Xingfus::Request.new('url')
...
describe Xingfus::Hydra do
  describe '#retry_codes' do
    it 'should be empty by default' do
      Xingfus::Hydra.new.retry_codes.should == []
    end
  ...
  describe '.new' do
    it 'should set the retry_codes with the :retry_codes option' do
      Xingfus::Hydra.new(:retry_codes => [505, 506]).retry_codes.should == [505, 506]
    end
  end

  describe '#disable_retry' do
    it 'should reset the retry codes to []' do
      hydra = Xingfus::Hydra.new(:retry_codes => [507, 508])
      hydra.disable_retry
  ...
  describe 'retry' do
    before(:each) do
      @request_timeout = 100
      Xingfus::Hydra.allow_net_connect = true
    ...
    describe 'with no retry configured' do
      ...
      it 'calls should not be retried' do
        Xingfus::Request.get("#{@flaky_prefix}/set?codes=503,200")
        response = Xingfus::Request.get("#{@flaky_prefix}?sanity-server-flip")
      ...
      describe 'server sanity check' do
        describe '/flaky' do
          ...
          it "it should return 503 multiple times if needed" do
            Xingfus::Request.get("#{@flaky_prefix}/set?codes=503,503,200")

            response = Xingfus::Request.get("#{@flaky_prefix}?sanity-server-list",
                                            :retry => false)
            response.code.should == 503
          ...
          retryable_methods.each do |method|
            [503, 504].each do |code|
              it "of type #{method} with #{code} should retry request" do
                retry_count = 0
                Xingfus::Request.get("#{@flaky_prefix}/set?codes=#{code},200")
                request = Xingfus::Request.new("#{@flaky_prefix}?single-#{code}-success",
                                               :method => method)
              ...
              it "should result in exception with multiple #{method} yielding #{code} responses" do
                Xingfus::Request.send(method, "#{@flaky_prefix}/set?codes=#{code},#{code},200")

                request = Xingfus::Request.new("#{@flaky_prefix}?#{code}-multiple-times-fails",
                                               :method => method)
                @hydra.queue request
                @hydra.run
          ...
          retryable_methods.each do |method|
            [503, 504].each do |code|
              it "#{method} with a single #{code} should be retried (#{memoization_state})" do
                ...
                callback_count = 0
                retry_count = 0
                request1 = Xingfus::Request.new("#{@flaky_prefix}?multi-request",
                                                :method => method)
                ...
                request2 = Xingfus::Request.new("#{@flaky_prefix}?multi-request",
                                                :method => method)
