...
describe Xingfus::HydraMock do
  it "should mark all responses as mocks" do
    response = Xingfus::Response.new(:mock => false)
    response.should_not be_mock

    mock = Xingfus::HydraMock.new("http://localhost", :get)
  ...
  describe "stubbing response values" do
    before(:each) do
      @stub = Xingfus::HydraMock.new('http://localhost:3000', :get)
    end
    ...
    describe "with multiple responses" do
      it "should return consecutive responses in the array, then keep returning the last one" do
        responses = []
        3.times do |i|
          responses << Xingfus::Response.new(:body => "response #{i}")
        end
  ...
  describe "#matches?" do
    describe "basic matching" do
      it "should not match if the HTTP verbs are different" do
        request = Xingfus::Request.new("http://localhost:3000",
                                       :method => :get)
        mock = Xingfus::HydraMock.new("http://localhost:3000", :post)
    ...
    describe "matching on ports" do
      it "should handle default port 80 sanely" do
        mock = Xingfus::HydraMock.new('http://www.example.com:80/', :get,
                                      :headers => { 'user-agent' => 'test' })
        request = Xingfus::Request.new('http://www.example.com/',
                                       :method => :get,
                                       :user_agent => 'test')
      ...
      it "should handle default port 443 sanely" do
        mock = Xingfus::HydraMock.new('https://www.example.com:443/', :get,
                                      :headers => { 'user-agent' => 'test' })
        request = Xingfus::Request.new('https://www.example.com/',
                                       :method => :get,
                                       :user_agent => 'test')
        mock.matches?(request).should be_true
      end
    ...
    describe "any HTTP verb" do
      it "should match any verb" do
        mock = Xingfus::HydraMock.new("http://localhost:3000", :any,
                                      :headers => { 'user-agent' => 'test' })
    ...
    describe "header matching" do
      def request(options = {})
        Xingfus::Request.new("http://localhost:3000", options.merge(:method => :get))
      end

      def hydra_mock(options = {})
        Xingfus::HydraMock.new("http://localhost:3000", :get, options)
      end
    ...
    describe "post body matching" do
      it "should not bother matching on body if we don't turn the option on" do
        request = Xingfus::Request.new("http://localhost:3000",
                                       ...
                                       :user_agent => 'test',
                                       :body => "fdsafdsa")
        mock = Xingfus::HydraMock.new("http://localhost:3000", :get,
                                      :headers => { 'user-agent' => 'test' })
        mock.matches?(request).should be_true
      end
      ...
      it "should regex match" do
        request = Xingfus::Request.new("http://localhost:3000/whatever/fdsa",
                                       :method => :get,
                                       :user_agent => 'test')
        mock = Xingfus::HydraMock.new(/fdsa/, :get,
                                      :headers => { 'user-agent' => 'test' })
        mock.matches?(request).should be_true
      end
    end
