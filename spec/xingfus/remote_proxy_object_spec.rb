...
describe Xingfus::RemoteProxyObject do
  before(:each) do
  ...
  it "should take a caller and call the clear_memoized_proxy_objects" do
    ...
    response = Xingfus::RemoteProxyObject.new(clear_proxy, @easy)
    response.code.should == 200
  end

  it "should take an easy object and return the body when requested" do
    response = Xingfus::RemoteProxyObject.new(lambda {}, @easy)
  ...
  it "should perform requests only on the first access" do
    response = Xingfus::RemoteProxyObject.new(lambda {}, @easy)
    response.code.should == 200
    Xingfus.should_receive(:perform_easy_requests).exactly(0).times
    response.code.should == 200
  end
  ...
  it "should call the on_success method with an easy object and proxy to the result of on_success" do
    ...
    k = Xingfus::RemoteProxyObject.new(lambda {}, @easy, :on_success => lambda {|e| klass.new(e)})
    k.blah.should == 200
  end
  ...
  it "should call the on_failure method with an easy object and proxy to the result of on_failure" do
    ...
    k = Xingfus::RemoteProxyObject.new(lambda {}, @easy, :on_failure => lambda {|e| klass.new(e)})
    k.blah.should == 0
  end
end
