...
describe Xingfus::Request do
  describe "#inspect" do
    before(:each) do
      @request = Xingfus::Request.new('http://www.google.com/',
                                      :body => "a=1&b=2",
                                      :params => { :c => 'ok' },
                                      :method => :get,
  ...
  describe "#localhost?" do
    %w(localhost 127.0.0.1 0.0.0.0).each do |host|
      it "should be true for the #{host} host" do
        req = Xingfus::Request.new("http://#{host}")
        req.should be_localhost
      end
    end

    it "should be false for other domains" do
      req = Xingfus::Request.new("http://google.com")
  ...
  describe "#params_string" do
    ...
    it "should accept symboled keys" do
      request = Xingfus::Request.new('http://google.com',
                                     :params => {
                                        :b => 'fdsa',
                                        :a => 'jlk',
                                        :c => '789'
  ...
  describe "quick request methods" do
    it "can run a GET synchronously" do
      response = Xingfus::Request.get("http://localhost:3000", :params => { :q => "hi"}, :headers => { :foo => "bar"})
      response.code.should == 200
      JSON.parse(response.body)["REQUEST_METHOD"].should == "GET"
    end

    it "can run a POST synchronously" do
      response = Xingfus::Request.post("http://localhost:3000", :params => { :q => { :a => "hi" } }, :headers => { :foo => "bar"})
      response.code.should == 200
    ...
    it "can run a PUT synchronously" do
      response = Xingfus::Request.put("http://localhost:3000", :params => { :q => "hi"}, :headers => { :foo => "bar"})
      response.code.should == 200
      JSON.parse(response.body)["REQUEST_METHOD"].should == "PUT"
    end

    it "can run a DELETE synchronously" do
      response = Xingfus::Request.delete("http://localhost:3000", :params => { :q => "hi"}, :headers => { :foo => "bar"})
      response.code.should == 200
  ...
  it "takes url as the first argument" do
    Xingfus::Request.new("http://localhost:3000").url.should == "http://localhost:3000"
  end

  it "should parse the host from the url" do
    Xingfus::Request.new("http://localhost:3000/whatever?hi=foo").host.should == "http://localhost:3000"
    Xingfus::Request.new("http://localhost:3000?hi=foo").host.should == "http://localhost:3000"
    Xingfus::Request.new("http://localhost:3000").host.should == "http://localhost:3000"
  end

  it "takes method as an option" do
    Xingfus::Request.new("http://localhost:3000", :method => :get).method.should == :get
  end

  it "takes headers as an option" do
    headers = {:foo => :bar}
    request = Xingfus::Request.new("http://localhost:3000", :headers => headers)
    request.headers.should == headers
  end

  it "takes params as an option and adds them to the url" do
    Xingfus::Request.new("http://localhost:3000", :params => { :foo => "bar"}).url.should == "http://localhost:3000?foo=bar"
  end

  it "takes request body as an option" do
    Xingfus::Request.new("http://localhost:3000", :body => "whatever").body.should == "whatever"
  end

  it "takes timeout as an option" do
    Xingfus::Request.new("http://localhost:3000", :timeout => 10).timeout.should == 10
  end

  it "takes cache_timeout as an option" do
    Xingfus::Request.new("http://localhost:3000", :cache_timeout => 60).cache_timeout.should == 60
  end

  it "takes follow_location as an option" do
    Xingfus::Request.new("http://localhost:3000", :follow_location => true).follow_location.should == true
  end

  it "takes max_redirects as an option" do
    Xingfus::Request.new("http://localhost:3000", :max_redirects => 10).max_redirects.should == 10
  end
  ...
  describe "time info" do
    it "should have time" do
      response = Xingfus::Request.get("http://localhost:3000")
